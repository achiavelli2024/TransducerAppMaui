    ...
    public class PhoenixTransducer : ITransducer
    {
        ...
        private bool bUserStartService
        {
            ...
            set
            {
                if (_bUserStartService != value)
                {
                    ...
                    try { TransducerLogAndroid.LogInfo("USER START SERVICE: {0}", value); } catch { }


                    // Also write system event to protocol log
                    //try { ProtocolFileLogger.WriteProtocol("SYS", "-- USER START SERVICE: " + value, null); } catch { }
                    //try { TransducerLogAndroid.LogRaw("SYS", "-- USER START SERVICE: " + value, null); } catch { }
        ...
        public List<Tuple<string, byte[]>> GetInitReadFrames()
        {
            ...
            catch (Exception ex)
            {
                ...
                try { TransducerLogAndroid.LogException(ex, "GetInitReadFrames failed"); } catch { }
        ...
        public void RequestInformation()
        {
            try
            {
                ...
                try { TransducerLogAndroid.LogInfo("RequestInformation called - MustSendRequestInformation set true"); } catch { }
            ...
            catch (Exception err)
            {
                ...
                try { TransducerLogAndroid.LogException(err, "RequestInformation exception"); } catch { }
                throw err;
            }
        }
        ...
        public void WriteSetup(DataInformation Info)
        {
            lock (objconfig)
            {
                ...
                try
                {
                    ...
                    try { TransducerLogAndroid.LogInfo("WriteSetup: flags.MustConfigure set true"); } catch { }

                }
                catch (Exception err)
                {
                    //TransducerLogger.LogException(err, "WriteSetup exception");
                    //try { TransducerLogger.LogException(err, "WriteSetup exception"); } catch { }
                    try { TransducerLogAndroid.LogException(err, "WriteSetup exception"); } catch { }
        ...
        private void dispatcherTimer_Tick(object sender, System.Timers.ElapsedEventArgs e)
        {
            try
            {
                ...
                //TransducerLogAndroid.LogDebug("aqui novo logTimerTick entry: state={0} tick_tx={1} tick_datareceived={2}", _state, ticks.tick_tx, ticks.tick_datareceived);
                ...
                lock (locker_State)
                {
                    ...
                    if (bPrintCommToFile)
                    {
                        try
                        {
                            ...
                            if (slog != slogprev)
                            {
                                ...
                                //try { TransducerLogAndroid.LogDebug("Flags snapshot: {0}", slog); } catch { }
                            }
                        }
                        catch { }
                    }
                    ...
                    lock (objconfig)
                    {
                        ...
                        if (_state == eState.eMustSendGetChartBlock)
                        {
                            if (_blocks.block != null)
                            {
                                ...
                                if (_blocks.block.Length > _blocks.indextoask)
                                {
                                    ...
                                    try
                                    {
                                        // extrai parâmetros do bloco
                                        int blockIndex = _blocks.indextoask;
                                        //int start = _blocks.block[blockIndex, DEF_COLUMN_STARTINDEX];
                                        //int blockSize = _blocks.block[blockIndex, DEF_COLUMN_SIZE];
                                        ...
                                        // Mensagem legível para UI/DB
                                        string humanMsg = $"Dispatch GD SEND: blockIndex={blockIndex} start={start} size={blockSize} step={step} awaited={awaited}";
                                        ...
                                        TransducerLogAndroid.LogInfo(humanMsg);

                                    }
                                    catch (Exception exLog)
                                    {
                                        //TransducerLogger.LogException(exLog, "Dispatch GD SEND logging failed");
                                        try { TransducerLogAndroid.LogException(exLog, "Dispatch GD SEND logging failed"); } catch { }
                                    }
                                ...
                                else
                                {
                                    ...
                                    TransducerLogAndroid.LogInfo("ALGORITHM PROBLEM: _blocks length mismatch");
                                }
                            }
                            else
                            {
                        ...
                        if (_state == eState.eMustSendReadCommand)
                        {
                            ...
                            TransducerLogAndroid.LogInfo("SendCommand_id + TQ");
                        }
                        ...
                        if (btimeouted)
                        {
                            if (_state != eState.eWaitingAquisitionAdditionalConfig && _state != eState.eWaitingAquisitionAdditional2Config)
                            {
                                lock (signalmeasure.lmeasure)
                                {
                                    ...
                                    TransducerLogAndroid.LogDebug("Timeout detected in state {0}", _state);
                                }
                            }
                        }
                        ...
                        if (bRestartTightening)
                        {
                            ...
                            TransducerLogAndroid.LogDebug("CANCELING CHART triggered");
                        ...
                        if ((_state != eState.eIdle && iConsecTimeout_OR_InvalidAnswer >= DEF_MAX_ERRS) || iTrashing > 3)
                        {
                            ...
                            TransducerLogAndroid.LogWarn("Too many timeouts/errors: iConsecTimeout_OR_InvalidAnswer={0} iTrashing={1}", iConsecTimeout_OR_InvalidAnswer, iTrashing);
        ...
        private bool Timeouted(int tick, int timeout, string printstring = null)
        {
            ...
            if (ret && printstring != null)
            {
                ...
                try { TransducerLogAndroid.LogDebug("Timeouted: {0} (tick={1} timeout={2})", printstring, tick, timeout); } catch { }
            ...
            for (int i = 0; i < 3; i++)
            {
                try
                {
                    if (((_Eth_IP == string.Empty || _Eth_IP == null)) || (_Eth_IP != string.Empty && _Eth_IP != null && sck.Connected))
                    {
                        ...
                        try { TransducerLogAndroid.LogDebug("TX -> {0}", cmdout); } catch { }
                        ...
                        try
                        {
                            ...
                            TransducerLogAndroid.LogDebug("TX BYTES: {0}", ByteArrayToHexString(txBytes));
                        ...
                        catch (Exception exHexLog)
                        {
                            ...
                            TransducerLogAndroid.LogException(exHexLog, "SendCommand: ProtocolFileLogger.WriteProtocol frame logging failed");
                        }
                        ...
                        try
                        {
                            ...
                            while (true)
                            {
                                ...
                                if (offset > 0)
                                {
                                    ...
                                    TransducerLogAndroid.LogDebug("RX raw all (len={0}): {1}", offset, sall);
                                    ...
                                    if (bspend > bspini && bspini > -1)
                                    {
                                        ...
                                        TransducerLogAndroid.LogDebug("PARSER candidate len={0} s='{1}'", s.Length, s);

                                        //TransducerLogger.LogHex("PARSER packet raw", bs, bspini, bspend - bspini + 1);
                                        TransducerLogAndroid.LogDebug("PARSER packet raw: {0}", ByteArrayToHexString(bs, " ", bspini, bspend - bspini + 1));
                                            ...
                                            )
                                        {
                                            try
                                            {
                                                ...
                                                TransducerLogAndroid.LogDebug("PARSER CRC calc={0} recv={1}", crcCalc, crcRecv);
                                                ...
                                                else
                                                {
                                                    ...
                                                    try { TransducerLogAndroid.LogRaw("PARSER wrong CRC, will wait: {0}", s); } catch { }
                                                }
                                            }
                                            catch (Exception ex)
                                            {
        ...
        public void StopReadData()
        {
            try
            {
                ...
                TransducerLogAndroid.LogInfo("StopReadData called - acquisition flags cleared");
            ...
            catch (Exception err)
            {
                ...
                TransducerLogAndroid.LogException(err, "StopReadData exception");
                throw err;
            }
        ...
        public void SetZeroTorque()
        {
            try
            {
                ...
                TransducerLogAndroid.LogDebug("SetZeroTorque called - MustSendZeroTorque true");
            ...
            catch (Exception err)
            {
                ...
                TransducerLogAndroid.LogException(err, "SetZeroTorque exception");
                throw err;
            }
        }
        public void SetZeroAngle()
        {
            try
            {
                ...
                TransducerLogAndroid.LogDebug("SetZeroAngle called - MustSendZeroAngle true");
            ...
            catch (Exception err)
            {
                ...
                TransducerLogAndroid.LogException(err, "SetZeroAngle exception");
                throw err;
            }
        }
        public void SetTorqueOffset(decimal torqueoffset)
        {
            try
            {
                ...
                TransducerLogAndroid.LogDebug($"SetTorqueOffset called - torqueoffset={torqueoffset}");
            ...
            catch (Exception err)
            {
                ...
                TransducerLogAndroid.LogException(err, "SetTorqueOffset exception");
                throw err;
            }
        }
        public void SetTestParameter(DataInformation Info, TesteType Type, ToolType toolType, decimal NominalTorque, decimal Threshold)
        {
            ...
            TransducerLogAndroid.LogInfo("SetTestParameter overload called (short)");
        }
        public void SetTestParameter(DataInformation Info, TesteType Type, ToolType toolType, decimal NominalTorque, decimal Threshold, decimal ThresholdEnd, int TimeoutEnd_ms, int TimeStep_ms, int FilterFrequency, eDirection direction)
        {
            ...
            TransducerLogAndroid.LogInfo("SetTestParameter overload called (mid)");
        }
        public void SetTestParameter(DataInformation Info, TesteType Type, ToolType toolType, decimal NominalTorque, decimal Threshold, decimal ThresholdEnd, int TimeoutEnd_ms, int TimeStep_ms, int FilterFrequency, eDirection direction, decimal TorqueTarget, decimal TorqueMin, decimal TorqueMax, decimal AngleTarget, decimal AngleMin, decimal AngleMax, int DelayToDetectFirstPeak_ms, int TimeToIgnoreNewPeak_AfterFinalThreshold_ms)
        {
            ...
            try
            {
                ...
                TransducerLogAndroid.LogDebug($"SetTestParameter: Threshold={Threshold} ThresholdEnd={ThresholdEnd} TimeStep_ms={TimeStep_ms} FilterFreq={FilterFrequency} direction={direction} " +
                    $"TorqueTarget={TorqueTarget} TorqueMin={TorqueMin} TorqueMax={TorqueMax} AngleTarget={AngleTarget} " +
                    $"AngleMin={AngleMin} AngleMax={AngleMax} DelayToDetectFirstPeak_ms={DelayToDetectFirstPeak_ms} " +
                    $"TimeToIgnoreNewPeak_AfterFinalThreshold_ms={TimeToIgnoreNewPeak_AfterFinalThreshold_ms}");
            }
            catch { }
        ...
        public void SetTestParameter_ClickWrench(ushort FallPercentage, ushort RisePercentage, ushort MinTimeBetweenPulses_ms)
        {
            ...
            TransducerLogAndroid.LogDebug($"SetTestParameter_ClickWrench called: Fall={FallPercentage} Rise={RisePercentage} MinTime_ms={MinTimeBetweenPulses_ms}");
        ...
        public void Calibrate(decimal AppliedTorque, decimal CurrentTorque, decimal AppliedAngle, decimal CurrentAngle)
        {
            try
            {
                ...
                TransducerLogAndroid.LogDebug($"Calibrate called: AppliedTorque={AppliedTorque} CurrentTorque={CurrentTorque} AppliedAngle={AppliedAngle} CurrentAngle={CurrentAngle}");
            ...
            catch (Exception err)
            {
                ...
                TransducerLogAndroid.LogException(err, "Calibrate exception");
                throw err;
            }
        }
        ...
        public void StartCommunication()
        {
            ...
            try
            {
                ...
                TransducerLogAndroid.LogDebug("StartCommunication called - MustSendGetID & MustSendGetCounters true");
            ...
            catch (Exception err)
            {
                ...
                TransducerLogAndroid.LogException(err, "StartCommunication exception");
                throw err;
            }
        }
    }
